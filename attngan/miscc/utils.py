...
def build_super_images(
    ...
    nvis = min(len(attn_maps), 8)  # number of visual
    ...
    for i in range(num):
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
                ...
                one_map = (one_map - minVglobal) / (maxVglobal - minVglobal + 1e7)
                one_map *= 255
                #
...
def build_super_images2(
    real_imgs, captions, cap_lens, ixtoword, attn_maps, att_sze, vis_size=256, topK=5
):
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            minV = one_map.min()
            maxV = one_map.max()
            one_map = (one_map - minV) / (maxV - minV + 1e7)
            row_beforeNorm.append(one_map)
