...
class TextDataset(data.Dataset):
    def __init__(
        ...
        self.filepath = data_dir + "/" + split + "/Fashiongen_256_256.h5"
        self.imgfile = None
        self.textfile = None
        self.classfile = None
        self.data = []
    ...
    def load_captions(self, dir):
        ...
        for i in range(int(num_batch)):
            batch_data = self.h5py_batch(
                file, "input_description", i, batch_size, num_batch
            )
    ...
    def load_subcas(self, dir):
        ...
        for i in range(int(num_batch)):
            batch_data = self.h5py_batch(
                file, "input_category", i, batch_size, num_batch
            )
            for j in range(batch_data.shape[0]):
    ...
    def build_dictionary(self, train_captions, test_captions):
        ...
        print(
            "{} vocabularies in fashion dataset".format(len(vocab))
    ...
    def build_dictionary2(self, train_subcas, test_subcas):
        ...
        sorted_subca_counts = OrderedDict(
            sorted(subca_counts.items(), key=lambda x: x[1], reverse=True)
        )
    ...
    def load_text_data(self):
        ...
        if not os.path.isfile(filepath):
            ...
            #             test_dir = "data/train/Fashiongen_256_256.h5"
    ...
    def load_class_id(self):
        ...
        if os.path.isfile(filepath):
            ...
            train_subcas, test_subcas = self.build_dictionary2(
                train_subcas, test_subcas
            )
            if self.split == "train":
    ...
    def __getitem__(self, index):  # call -- dataset[index]
        ...
        imgs = get_imgs(
            self.imgfile[index], self.imsize, self.bbox, self.transform, self.norm
        )
